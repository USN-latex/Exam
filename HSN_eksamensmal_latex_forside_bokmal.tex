...
%%%% ----- Start bokmål forside -------------- % Den engelske er under, merka og kommentert ut
\begin{document}
	\newgeometry{top = 2.5cm, bottom=1.5cm, left=1.5cm, right=1.5cm,includefoot}
	\maketitle
	\normalsize
	\thispagestyle{empty}    % Fjernar sidetal på framsida 
        \pagestyle{plain}        % Måtte ha denne for at det ikke skulle stå "Page 2", men bare "2".
...
%			  &
		...
		\begin{tabularx}{\textwidth}{|X X X|}	
			...
			Ruter  & Linjer & Blanke    \\ %INFO: Valg av papirtype her
			\hline 
		\end{tabularx}
		
		\vspace{0.2cm}  % minska fra 0.5cm for å få hele forsida på ei side.
		\noindent
